...
function DiseasePanel() {
  ...
  return (
    <div>
      ...
      {/* Sub-tabs */}
      <div style={{ display: "flex", marginBottom: 20 }}>
        {[["snapshot", "2021 Snapshot"], ["trend", "Cost Trends 2000–2021"], ["inflation", "Cost Inflation"], ["chapters", "By Chapter"], ["broken", "⚠ The Broken System"]].map(([id, lbl], i, arr) => (
          <button key={id} className={`tab-btn${view === id ? " active" : ""}`}
            style={{ borderRadius: i === 0 ? "4px 0 0 4px" : i === arr.length - 1 ? "0 4px 4px 0" : "0", borderLeft: i > 0 ? "none" : undefined }}
            onClick={() => setView(id)}>{lbl}</button>
        ))}
      ...
      {view === "broken" && (
        <div>
          ...
          {/* Row 2: Waste breakdown + equity callouts */}
